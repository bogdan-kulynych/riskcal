...
def get_rdp(noise_multiplier, epochs, batch_size=8192, data_size=50000, orders=ORDERS):
    ...
    rdp_sgd = get_renyi_divergence(sample_rate, noise_multiplier, orders=orders) * steps * epochs
...
for i, row in tqdm.tqdm(list(exp_metadata.iterrows())):
    rdp = get_rdp(noise_multiplier=row.sigma, epochs=row.epochs, batch_size=row.batch_size)

    # CF delta
    cf_eps = get_privacy_spent(rdp, cf_delta)

    for alpha in alphas:
        cf_beta = riskcal.utils.get_err_rate_for_epsilon_delta(cf_eps, cf_delta, alpha=alpha)
        ...
        plot_data.append(dict(
            alpha=alpha,
            cf_beta=cf_beta, cf_tpr=1 - cf_beta, cf_eps=cf_eps,
            cal_eps=cal_eps, cal_delta=cal_delta, cal_beta=cal_beta, cal_tpr=1 - cal_beta,
...
g = sns.relplot(
    data=(
        ...
        .melt(id_vars=["alpha", "test_acc", "sigma"],
              value_vars=["cal_tpr", "cf_tpr"],
              var_name="Method",
              value_name="TPR (attack sensitivity)"
        )
        .replace({"cal_tpr": "TPR/FPR calibration", "cf_tpr": "Standard CF calibration"})
        .rename(columns={
            "alpha": "FPR",
            "test_acc": "Test acc.",
            "sigma": "Noise scale",
